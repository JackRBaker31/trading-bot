...
FILES = (
    "app/opportunity_ranking_history_models.py",
    "app/opportunity_ranking_history_repository.py",
    "app/opportunity_ranking_history_service.py",
    "app/opportunity_ranking_service.py",
    "app/intelligence_cycle_service.py",
    "app/job_executor.py",
    "app/run_job_worker.py",
    "app/supervisor_status_repository.py",
    "web/dependencies.py",
    "web/app.py",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install KAIRO Opportunity History & Rank Drift v0.10."
    )
    parser.add_argument(
        "project_root",
        nargs="?",
...
def validate_baseline(project_root: Path) -> None:
    required = (
        project_root / "app" / "opportunity_ranking_models.py",
    ...
    if missing:
        raise RuntimeError(
            "KAIRO v0.9 must be installed first. Missing: "
            + ", ".join(missing)
        )
...
def main() -> None:
    ...
    backup_root = (
        project_root
        / "source-backups"
        / f"opportunity-history-v0.10-{timestamp}"
    )
    ...
    print("KAIRO Opportunity History & Rank Drift v0.10 installation complete.")
    print("The SQLite history table will be created automatically at startup.")
