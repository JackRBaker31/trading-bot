...
FILES = (
    ...
    "frontend/src/__tests__/supervisor-status.test.tsx",
    "tests/test_run_kairo.py",
    "Start Everything.bat",
    "Restart Everything.bat",
    "check_kairo_status.bat",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install KAIRO Supervisor Unification v0.8.1."
    )
    parser.add_argument(
        "project_root",
        nargs="?",
...
def validate_baseline(project_root: Path) -> None:
    required = (
        ...
        project_root / "app" / "infrastructure_status_service.py",
        project_root / "frontend" / "src" / "pages" / "OperationsPage.tsx",
        project_root / "frontend" / "src" / "pages" / "DashboardPage.tsx",
        project_root / "run_kairo.py",
    )
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise RuntimeError(
            "KAIRO Market Data Resilience v0.8 must be installed first. Missing: "
            + ", ".join(missing)
        )
...
def ensure_supervisor_stopped(project_root: Path) -> None:
    ...
    if isinstance(pid, int) and pid_is_running(pid):
        raise RuntimeError(
            "The current KAIRO watchdog/supervisor is still running. "
            "Press Ctrl+C in its window, then run Stop Everything.bat before installing."
        )
...
def main() -> None:
    ...
    backup_root = (
        project_root
        / "source-backups"
        / f"supervisor-unification-v0.8.1-{timestamp}"
    )
    ...
    legacy_pid = project_root / "data" / "kairo_supervisor.pid"
    if legacy_pid.exists():
        runtime_backup = backup_root / "runtime" / "kairo_supervisor.pid"
        runtime_backup.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(legacy_pid, runtime_backup)
        legacy_pid.unlink(missing_ok=True)
        print("Removed stale legacy supervisor PID file.")

    print()
    print("Source backup:", backup_root)
    print("KAIRO Supervisor Unification v0.8.1 installation complete.")
