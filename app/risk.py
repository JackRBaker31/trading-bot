...
class RiskEngine:
    ...
    def evaluate(
        ...
        if order.side == OrderSide.BUY:
            return self._evaluate_buy(order, portfolio, current_prices)

        return self._evaluate_sell(order, portfolio)
    ...
    def _evaluate_buy(
        ...
        existing_quantity = portfolio.positions.get(order.symbol, 0)
        existing_price = current_prices.get(order.symbol, order.price)
    ...
    def _evaluate_sell(
        ...
        owned_quantity = portfolio.positions.get(order.symbol, 0)
