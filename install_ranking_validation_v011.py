...
FILES = (
    "app/opportunity_ranking_validation_models.py",
    "app/opportunity_ranking_validation_repository.py",
    "app/opportunity_ranking_validation_service.py",
    "app/intelligence_cycle_service.py",
    "app/job_executor.py",
    "app/run_job_worker.py",
    "web/dependencies.py",
    "web/app.py",
    "frontend/src/hooks/useRankingValidation.ts",
    "frontend/src/pages/RankingValidationPage.tsx",
    ...
    "frontend/src/__tests__/ranking-validation.test.tsx",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install KAIRO Ranking Validation & Forward Returns v0.11."
    )
    parser.add_argument(
        "project_root",
        nargs="?",
...
def validate_baseline(project_root: Path) -> None:
    required = (
        project_root / "app" / "opportunity_ranking_history_models.py",
        project_root / "app" / "opportunity_ranking_history_repository.py",
        project_root / "app" / "opportunity_ranking_history_service.py",
        project_root / "app" / "opportunity_ranking_service.py",
        project_root / "frontend" / "src" / "pages" / "OpportunityRankingPage.tsx",
        project_root / "frontend" / "src" / "hooks" / "useOpportunityRanking.ts",
        project_root / "run_kairo.py",
    )
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise RuntimeError(
            "KAIRO v0.10 must be installed first. Missing: "
            + ", ".join(missing)
        )
...
def initialize_database(project_root: Path) -> None:
    ...
    database_path = str(project_root / "data" / "application.db")
    OpportunityRankingHistoryRepository(
...
def main() -> None:
    ...
    backup_root = (
        project_root
        / "source-backups"
        / f"ranking-validation-v0.11-{timestamp}"
    )
    ...
    print("Database table: opportunity_ranking_forward_outcomes")
    print("KAIRO Ranking Validation & Forward Returns v0.11 installation complete.")
